...
public class LearningSessionRestController {
    ...
    @PreAuthorize("isAuthenticated()")
    public ResponseEntity<?> getSessionForManagement(
            @PathVariable Long id,
            HttpServletRequest request) {
        try {
            Authentication authentication = SecurityContextHolder.getContext().getAuthentication();
            ...
            LearningSession session = learningSessionService.getSessionByIdForInstructor(id, authenticatedPerson);

            return new GlobalResponseHandler().handleResponse(
                    "Session retrieved for management successfully",
                    session,
                    HttpStatus.OK,
                    request
            );
            ...
            System.err.println("Error getting session for management: " + e.getMessage());
            e.printStackTrace();
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                    .body(createErrorResponse("Error retrieving session",
                            "Error al obtener la sesión para gestión: " + e.getMessage()));
        }
    }
    //#endregion
